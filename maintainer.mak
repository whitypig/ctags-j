...
COMMON_FILES := COPYING EXTENDING.html FAQ INSTALL.oth MAINTAINERS NEWS README README_J.TXT \
...
UNIX_FILES   := $(COMMON_FILES) \
				...
				ctags.spec ctags.1 ctags_j.html
...
$(RELEASE_DIR)/ctags-%.tar.gz: $(UNIX_FILES) | $(RELEASE_DIR)
	...
	(cd $(@D)/dirs/ctags-$* ;\
		...
		autoconf ;\
		sed -e "s/^\(DEFS=\)\(.*\)/\1'\2 -DKANJI'/" configure > configure.temp ;\
...
$(WINDOWS_DIR)/ctags$(win_version): \
	...
	cp -p ctags_j.html "$(WINDOWS_DIR)/ctags$(win_version)"
	chmod 644 $@/*
	chmod 755 $@/$(REGEX_DIR)
	chmod 644 $@/$(REGEX_DIR)/*
